...
void *worker(void *arg) {
    struct epoll_dispatcher *poll_dis = (struct epoll_dispatcher *)arg;

    printf("poll-size: %d\n", poll_dis->size);

    epoll_dispatch(poll_dis);
...
void accept_sock(int serv_fd) {
    ...
    struct epoll_dispatcher *poll_dis = epoll_new();
